/* Final Demo - código adaptado de ObjectTrajectories.cpp e https://learnopengl.com/#!Getting-started/Hello-Triangle
 *
...
void renderObject3D(const Object3D &object, GLuint objectVAO, GLint modelMatrixLocation, int vertexCount);
...
int main()
{
  ...
  GLFWwindow *window = glfwCreateWindow(WIDTH, HEIGHT, "Final Demo -- Gabriela Bado", nullptr, nullptr);
  ...
  gShaderID = shaderID;
  GLuint suzanneVAO = loadSimpleOBJ("../assets/Modelos3D/Suzanne/Suzanne.obj", suzanneVertices);
  string suzanneTextureName = loadTexturePathFromMTL("../assets/Modelos3D/Suzanne/Suzanne.mtl");
  GLuint suzanneTexture = loadTexture("../assets/Modelos3D/Suzanne/" + suzanneTextureName);
  Material suzanneMaterial = loadMaterialFromMTL("../assets/Modelos3D/Suzanne/Suzanne.mtl");

  GLuint cubeVAO = loadSimpleOBJ("../assets/Modelos3D/Cube.obj", cubeVertices);
  string cubeTextureName = loadTexturePathFromMTL("../assets/Modelos3D/Cube.mtl");
  GLuint cubeTexture = loadTexture("../assets/Modelos3D/" + cubeTextureName);
  Material cubeMaterial = loadMaterialFromMTL("../assets/Modelos3D/Cube.mtl");
  ...
  while (!glfwWindowShouldClose(window))
  {
    ...
    glBindTexture(GL_TEXTURE_2D, suzanneTexture);

    renderObject3D(objects[0], suzanneVAO, modelMatrixLocation, suzanneVertices);

    uploadMaterialToShader(shaderID, cubeMaterial);

    glBindTexture(GL_TEXTURE_2D, cubeTexture);
    renderObject3D(objects[1], cubeVAO, modelMatrixLocation, cubeVertices);

    glfwSwapBuffers(window);
  }
  glDeleteTextures(1, &suzanneTexture);
...
void showControlsGuide()
{
  cout << endl;
  cout << "==========================================" << endl;
  cout << " Bem-vindo a Final Demo! " << endl;
